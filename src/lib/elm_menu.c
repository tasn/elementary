   ...
   if (item->parent)
     {
        ...
        eo_do(ELM_INTERFACE_ATSPI_ACCESSIBLE_MIXIN, elm_interface_atspi_accessible_event_emit(EO_OBJ(item->parent), ELM_INTERFACE_ATSPI_SELECTION_EVENT_SELECTION_CHANGED, NULL));
     }
   else
     {
...
_item_submenu_obj_create(Elm_Menu_Item_Data *item)
{
   ...
   eo_do(item->submenu.location, elm_interface_atspi_accessible_disabled_set(EINA_TRUE));
   item->submenu.hv = hv = elm_hover_add(sd->bx);
   eo_do(item->submenu.hv, elm_interface_atspi_accessible_disabled_set(EINA_TRUE));
...
_elm_menu_evas_object_smart_add(Eo *obj, Elm_Menu_Data *priv)
{
   ...
   eo_do(priv->location, elm_interface_atspi_accessible_disabled_set(EINA_TRUE));

   priv->hv = elm_hover_add(obj);
   eo_do(priv->hv, elm_interface_atspi_accessible_disabled_set(EINA_TRUE));
   ...
   eo_do(priv->bx, elm_interface_atspi_accessible_disabled_set(EINA_TRUE));
...
_elm_menu_item_add(Eo *obj, Elm_Menu_Data *sd, Elm_Object_Item *parent, const char *icon, const char *label, Evas_Smart_Cb func, const void *data)
{
   ...
   eo_do(icon_obj, elm_interface_atspi_accessible_disabled_set(EINA_TRUE));
...
{
   Eina_List *ret;
   eo_do_super(obj, ELM_MENU_CLASS, ret = elm_interface_atspi_accessible_children_get());
...
_elm_menu_item_elm_interface_atspi_accessible_state_set_get(Eo *obj EINA_UNUSED, Elm_Menu_Item_Data *sd)
{
   ...
   eo_do_super(obj, ELM_MENU_ITEM_CLASS, ret = elm_interface_atspi_accessible_state_set_get());
