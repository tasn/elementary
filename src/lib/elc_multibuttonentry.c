             ...
             if ((w < 0) || (w < w_label_count))
               {
                  ...
                  eo_do(obj, eo_event_callback_call
                    (ELM_MULTIBUTTONENTRY_EVENT_EXPAND_STATE_CHANGED, (void *)1));
                  break;
               }
          }
     }
...
_mouse_clicked_signal_cb(void *data EINA_UNUSED,
                         ...
                         const char *source EINA_UNUSED)
{
   eo_do(obj, eo_event_callback_call(ELM_MULTIBUTTONENTRY_EVENT_CLICKED, NULL));
}
               ...
               void *event EINA_UNUSED)
{
   ...
   if (sd->h_box < h)
     ...
     eo_do(sd->parent, eo_event_callback_call
       (ELM_MULTIBUTTONENTRY_EVENT_CONTRACTED, NULL));
...
_callbacks_register(Evas_Object *obj)
{
   ...
   eo_do(sd->entry, eo_event_callback_add
     (ELM_MULTIBUTTONENTRY_EVENT_CLICKED, _entry_clicked_cb, obj));
}
