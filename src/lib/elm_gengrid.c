                  ...
                  void *event_info)
{
   ...
   if (eo_do((Eo *)EO_OBJ(it), elm_wdg_item_disabled_get()) || (dragged)) return;
...
_elm_gengrid_item_focused(Elm_Gen_Item *it)
{
   ...
   switch (_elm_config->focus_autoscroll_mode)
     {
      case ELM_FOCUS_AUTOSCROLL_MODE_SHOW:
         elm_gengrid_item_show
            (EO_OBJ(it), ELM_GENGRID_ITEM_SCROLLTO_IN);
         break;
   ...
   if ((focus_raise) && (!strcmp(focus_raise, "on")))
     evas_object_raise(VIEW(it));
   evas_object_smart_callback_call(obj, SIG_ITEM_FOCUSED, EO_OBJ(it));
}
...
_elm_gengrid_item_unfocused(Elm_Gen_Item *it)
{
   ...
   evas_object_smart_callback_call(obj, SIG_ITEM_UNFOCUSED, EO_OBJ(it));
}
...
_item_focus_up(Elm_Gengrid_Data *sd)
{
   ...
   if (!sd->focused_item)
     ...
     {
        Elm_Object_Item *eo_prev = elm_gengrid_item_prev_get(EO_OBJ(sd->focused_item));
        if (!eo_prev) return EINA_FALSE;
        prev = eo_data_scope_get((Eo *)eo_prev, ELM_GENGRID_ITEM_CLASS);
        if (prev == sd->focused_item) return EINA_FALSE;
     }
...
_item_focus_down(Elm_Gengrid_Data *sd)
{
   ...
   if (!sd->focused_item)
     ...
     {
        Elm_Object_Item *eo_next = elm_gengrid_item_next_get(EO_OBJ(sd->focused_item));
        if (!eo_next) return EINA_FALSE;
        next = eo_data_scope_get((Eo *)eo_next, ELM_GENGRID_ITEM_CLASS);
        if (next == sd->focused_item) return EINA_FALSE;
     }
...
_item_multi_select_left(Elm_Gengrid_Data *sd)
{
   ...
   eo_prev = elm_gengrid_item_prev_get(EO_OBJ(sd->last_selected_item));
   if (!eo_prev) return EINA_TRUE;

   if (elm_gengrid_item_selected_get(eo_prev))
     {
        ELM_GENGRID_ITEM_DATA_GET(eo_prev, prev);
        elm_gengrid_item_selected_set(EO_OBJ(sd->last_selected_item), EINA_FALSE);
...
_item_multi_select_right(Elm_Gengrid_Data *sd)
{
   ...
   eo_next = elm_gengrid_item_next_get(EO_OBJ(sd->last_selected_item));
   if (!eo_next) return EINA_TRUE;

   if (elm_gengrid_item_selected_get(eo_next))
     {
        ELM_GENGRID_ITEM_DATA_GET(eo_next, next);
        elm_gengrid_item_selected_set(EO_OBJ(sd->last_selected_item), EINA_FALSE);
   ...
   while (prev)
     {
        for (i = 0; i < sd->nmax; i++)
          {
             Elm_Object_Item *eo_prev = elm_gengrid_item_prev_get(EO_OBJ(prev));
             if (!eo_prev) return EINA_FALSE;
             prev = eo_data_scope_get((Eo *)eo_prev, ELM_GENGRID_ITEM_CLASS);
          }
   ...
   while (next)
     {
        for (i = 0; i < sd->nmax; i++)
          {
             Elm_Object_Item *eo_next = elm_gengrid_item_next_get(EO_OBJ(next));
             if (!eo_next) return EINA_FALSE;
             next = eo_data_scope_get((Eo *)eo_next, ELM_GENGRID_ITEM_CLASS);
          }
...
_item_single_select_left(Elm_Gengrid_Data *sd)
{
   ...
   if (!sd->selected)
     ...
     {
        Elm_Object_Item *eo_prev =
           elm_gengrid_item_prev_get(EO_OBJ(sd->last_selected_item));
        while (eo_prev)
          {
...
_item_single_select_right(Elm_Gengrid_Data *sd)
{
   ...
   if (!sd->selected)
     ...
     {
        Elm_Object_Item *eo_next =
           elm_gengrid_item_next_get(EO_OBJ(sd->last_selected_item));
        while (eo_next)
          {
...
static Eina_Bool
_elm_gengrid_item_edge_check(Elm_Gen_Item *it,
...
get_up_item(Elm_Gengrid_Data *sd, Elm_Gen_Item *it)
{
   Elm_Object_Item *eo_prev = NULL;
   unsigned int i;

   eo_prev = elm_gengrid_item_prev_get(EO_OBJ(it));
   if (!eo_prev)
     return NULL;
...
static Elm_Object_Item *
get_down_item(Elm_Gengrid_Data *sd, Elm_Gen_Item *it)
{
   Elm_Object_Item *eo_next = NULL;
   unsigned int i;

   eo_next = elm_gengrid_item_next_get(EO_OBJ(it));
   if (!eo_next)
     return NULL;
...
_item_horizontal_loop(Evas_Object *obj, Elm_Focus_Direction dir)
{
   ...
   if (sd->reorder_mode && !(EO_OBJ(sd->focused_item) == eo_item))
     {
        item = eo_data_scope_get((Eo *)eo_item, ELM_GENGRID_ITEM_CLASS);
        _swap_items(sd->focused_item, item, ELM_FOCUS_RIGHT);
        return EINA_TRUE;
     }
   else
     {
   ...
   if (!strcmp(dir, "left"))
        ...
        if (sd->reorder_mode)
          {
             ...
             if (!sd->horizontal)
               eo_left = elm_gengrid_item_prev_get(EO_OBJ(sd->focused_item));
             else
               eo_left = get_up_item(sd, sd->focused_item);
             if (!eo_left)
               return EINA_TRUE;
             ELM_GENGRID_ITEM_DATA_GET(eo_left, left);
             _swap_items(sd->focused_item, left, ELM_FOCUS_LEFT);
        ...
        if (sd->horizontal)
        ...
        else
          {
             if (sd->focused_item && (_elm_gengrid_item_edge_check(sd->focused_item, ELM_FOCUS_LEFT) ||
                 !elm_gengrid_item_prev_get(EO_OBJ(sd->focused_item))))
               {
                  if (sd->item_loop_enable)
                    {
                       if (_item_horizontal_loop(obj, ELM_FOCUS_RIGHT))
   ...
   else if (!strcmp(dir, "right"))
     {
        if (sd->reorder_mode)
          {
             ...
             if (!sd->horizontal)
               eo_right = elm_gengrid_item_next_get(EO_OBJ(sd->focused_item));
             else
               eo_right = get_down_item(sd, sd->focused_item);
             if (!eo_right)
               return EINA_TRUE;
             ELM_GENGRID_ITEM_DATA_GET(eo_right, right);
             _swap_items(sd->focused_item, right, ELM_FOCUS_RIGHT);
        ...
        if (sd->horizontal)
          ...
          {
             if (sd->focused_item && (_elm_gengrid_item_edge_check(sd->focused_item, ELM_FOCUS_RIGHT) ||
                 !elm_gengrid_item_next_get(EO_OBJ(sd->focused_item))))
               {
                  if (sd->item_loop_enable)
                    {
                       if (_item_horizontal_loop(obj, ELM_FOCUS_LEFT))
        ...
        if (sd->reorder_mode)
          {
             ...
             if (!sd->horizontal)
               ...
               eo_up = elm_gengrid_item_prev_get(EO_OBJ(sd->focused_item));
             if (!eo_up)
               return EINA_TRUE;
             ELM_GENGRID_ITEM_DATA_GET(eo_up, up);
             _swap_items(sd->focused_item, up, ELM_FOCUS_UP);
        ...
        if (sd->reorder_mode)
          {
             ...
             if (!sd->horizontal)
               ...
               eo_down = elm_gengrid_item_next_get(EO_OBJ(sd->focused_item));
             if (!eo_down)
               return EINA_TRUE;
             ELM_GENGRID_ITEM_DATA_GET(eo_down, down);
             _swap_items(sd->focused_item, down, ELM_FOCUS_DOWN);
 ...
 */
static Elm_Gen_Item *
_elm_gengrid_nearest_visible_item_get(Evas_Object *obj, Elm_Gen_Item *it)
{
   ...
   if (elm_widget_focus_get(obj) && !sd->mouse_down)
     {
        if (sd->last_focused_item)
          ...
          {
             Elm_Object_Item *eo_it = elm_gengrid_first_item_get(obj);
             if (eo_it) it = eo_data_scope_get((Eo *)eo_it, ELM_GENGRID_ITEM_CLASS);
             else it = NULL;
             is_sel = EINA_TRUE;
          }
             ...
             if (it)
               {
                  if (!_elm_config->item_select_on_focus_disable && is_sel)
                    elm_gengrid_item_selected_set(EO_OBJ(it), EINA_TRUE);
                  else
                    elm_object_item_focus_set(EO_OBJ(it), EINA_TRUE);
...
_elm_gengrid_item_del_not_serious(Elm_Gen_Item *it)
{
   ...
   eo_do((Eo *)EO_OBJ(it), elm_wdg_item_pre_notify_del());
...
EOLIAN static void
_elm_gengrid_item_elm_widget_item_focus_set(Eo *eo_it EINA_UNUSED, Elm_Gen_Item *it, Eina_Bool focused)
{
   ELM_GENGRID_ITEM_CHECK_OR_RETURN(it);
   Evas_Object *obj = WIDGET(it);
   ELM_GENGRID_DATA_GET(obj, sd);
   ...
   if (focused)
     ...
     _elm_gengrid_item_unfocused(it);
}

EOLIAN static Eina_Bool
_elm_gengrid_item_elm_widget_item_focus_get(Eo *eo_it EINA_UNUSED, Elm_Gen_Item *it)
{
   ELM_GENGRID_ITEM_CHECK_OR_RETURN(it, EINA_FALSE);
   Evas_Object *obj = WIDGET(it);
   ELM_GENGRID_DATA_GET(obj, sd);
...
EOLIAN static void
_elm_gengrid_item_bring_in(Eo *eo_it EINA_UNUSED, Elm_Gen_Item *it, Elm_Gengrid_Item_Scrollto_Type type)
{
   ELM_GENGRID_DATA_GET_FROM_ITEM(it, sd);
