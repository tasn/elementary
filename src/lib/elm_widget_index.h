...
#define ELM_INDEX_ITEM_DATA(o) \
  ((Elm_Index_Item_Data*)eo_data_scope_get(o, ELM_INDEX_ITEM_CLASS))

#define ELM_INDEX_ITEM_CHECK(obj)                            \
  if (EINA_UNLIKELY(!eo_isa((obj), ELM_INDEX_ITEM_CLASS))) \
    return

#define ELM_INDEX_ITEM_CHECK_OR_RETURN(obj, ...)                        \
  if (EINA_UNLIKELY(!eo_isa((obj), ELM_INDEX_ITEM_CLASS))) \
    return __VA_ARGS__;
