   ...
   if (!go)
     {
        ...
        eo_do(obj, eo_event_callback_call(ELM_PHOTOCAM_EVENT_ZOOM_STOP, NULL));
     }
...
_elm_photocam_zoom_set(Eo *obj, Elm_Photocam_Data *sd, double zoom)
{
   ...
   if (!sd->paused)
     {
        if (started)
          eo_do(obj, eo_event_callback_call(ELM_PHOTOCAM_EVENT_ZOOM_START, NULL));
        if (!an)
          eo_do(obj, eo_event_callback_call(ELM_PHOTOCAM_EVENT_ZOOM_STOP, NULL));
     }
   if (zoom_changed)
     eo_do(obj, eo_event_callback_call(ELM_PHOTOCAM_EVENT_ZOOM_CHANGE, NULL));
}
